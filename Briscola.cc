...
extern "C" void carta_button_clicked(GtkButton *button, gpointer data){
	...
	/* Caso in cui sia il turno del giocatoreA.
	 */
	if(gioca == giocatoreA){
		// Prende il rif all'immagine sul tavolo.
		GtkImage *image = GTK_IMAGE(gtk_builder_get_object(builder,"image2"));
	...
	/* Caso in cui sia il turno del giocatoreB.
	 */
	if(gioca == giocatoreB){
		// Decrementa l'indice in quanto i nomi delle label partono da 3, 
		// invece che da zero.
		indice=indice-3;
		// Prende il rif all'immagine sul tavolo.
		GtkImage *image = GTK_IMAGE(gtk_builder_get_object(builder,"image3"));
		// Setta image 3 con l'immagine giusta del bottone premuto.
		gtk_image_set_from_file(image,partita.player2.mano[indice].nome);	
		// Setta tale immagine a visible.
		gtk_widget_set_visible(GTK_WIDGET(image),true);	
		//gtk_widget_set_visual(GTK_WIDGET(image), gtk_widget_get_visual(GTK_WIDGET(image)));
		//cout<<std::flush;
		partita.player2.mano[indice].selez=true;
